...
Assistant replies now include direct copy, regenerate, and on-device read-aloud actions. Regenerate replaces the most recent assistant response using the existing conversation context. The latest user message can be returned to the composer for editing and resending. The composer also supports system speech recognition after an Android microphone-permission prompt. Native Markdown presentation now handles headings, bullets, and fenced code blocks; richer tables, math, and Mermaid remain future polish.
...
Conversation backup is available now through user-selected JSON export and import. Imports merge valid mobile-format conversations by ID. Backups deliberately exclude all provider credentials; extension-schema compatibility is subsequent work.
